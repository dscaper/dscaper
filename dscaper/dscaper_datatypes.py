...
class DscaperLabel(BaseModel):
    library: str
    label: str
    description: Optional[str] = None
    sandbox: str = "{}"  # JSON string
    audios: list[DscaperAudio] = []  # set by the server
...
class DscaperEvent(BaseModel):
    ...
    event_duration: Optional[list[str]] = None  # if not set, will use
    # duration of the audio file or default to 5 seconds
...
class DscaperGenerate(BaseModel):
    ...
    id: Optional[str] = None  # set by the server
    timestamp: int = 0  # set by the server
    generated_files: list[str] = []  # List of generated audio files,
    # set by the server
...
class DscaperApiResponse(BaseModel):
    ...
    media_type: Optional[str] = "text/plain"  # Optional type field for
